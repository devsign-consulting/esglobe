...
router.post('/', async function(req, res, next) {
    ...
    const formModel = transformFormModel(req.body);
    req.body.layout = null;

    console.log("===formModel MODEL ===", JSON.stringify(formModel, null, 4));
    res.render('formBuilder', formModel, function (err, html) {
        if (err) res.status(500).send(err);
        console.log("===formModel MODEL HTML===", html);
        res.send({ html });
    });
});

function _camelCaseConvert(input) {
    const result = input.replace(/([A-Z])/g, " $1");
    const finalResult = result.charAt(0).toUpperCase() + result.slice(1); // capitalize the first letter - as an example.
    return finalResult;
}

function _processDropdown(input) {
    const output = [];
...
function transformFormModel(model) {
    let count = 0;
    let row = {};
    const modelArray = [];
    ...
    _.forEach(model.rows, row => {
        _.forEach(row, (val, key) => {
            ...
            if (val.association && (val.type === "dropdown" || val.type === 'multiselect')) {
                if (model.associations[val.association] && _.isFunction(model.associations[val.association].then)) {
                    model.associations[val.association].then(result => {
                        val.data = result;
                        val.dropdown = _processDropdown(val);
            ...
            if (!val.label) val.label = _camelCaseConvert(key);
            val.name = key;
